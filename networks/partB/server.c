...
// random number generator to return 2, 3, 5, 7, 11
int random_number() {
    int random = rand() % 5;
    ...
    while (1) {
        ...
        for (int n = 0; n < nfds; ++n) {
            if (events[n].data.fd == sockfd) {
                int len = recvfrom(sockfd, buffer, MAX_BUFFER, 0, (struct sockaddr*)&client_addr, &client_len);
                if (len > 0) {
                    ...
                    if (sending_data) {
                        ...
                        sendto(sockfd, &received_chunk, sizeof(chunk), 0, (struct sockaddr*)&client_addr, client_len);
                        ...
                        // // go through linked list of chunks and check if all chunks are received
        ...
        if (sending_data && chunk_list != NULL) {
            ...
            while (current != NULL) {
                ...
                if (diff_ms >= RETRANSMISSION_TIMEOUT_MS) {
                    sendto(sockfd, &(current->data), sizeof(chunk), 0, (struct sockaddr*)&client_addr, client_len);
